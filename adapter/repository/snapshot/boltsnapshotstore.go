...
func (f *BoltSnapshotStore) getMetaFromDB(id string) (*raft.SnapshotMeta, error) {
	...
	filename := filepath.Join(f.path, id, f.databaseFilename)
	...
	err = boltDB.View(func(tx *bolt.Tx) error {
		...
		if val != nil {
			var snapshotIndexes raft.IndexValue
...
func (s *BoltSnapshotSink) writeBoltDBFile() error {
	...
	dbPath := filepath.Join(path, s.store.databaseFilename)
